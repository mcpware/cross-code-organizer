/**
 * Session Distiller V3 — Extractive filtering for Claude Code sessions.
 *
 * V3 changes from V2 (based on senior engineer audit):
 * - P0: Fix dangling tool_results — never drop tool-only messages
 * - P0: Fix MCP tool input extraction (selector, text, key, todos)
 * - P1: Use tool_use_id matching instead of lastToolName (parallel calls safe)
 * - P1: Strip JSON envelope redundancy instead of stripping more content
 * - P2: Agent results 2000 chars, thinking first 200 chars, Bash head+tail
 * - Target: 60-80% reduction (not 90%+)
 */
    ...
    switch (b.type) {
      ...
      case "tool_result": {
        ...
        if (toolName === "Read") {
          ...
          // P2: head 5 + tail 5 (keeps headers AND errors)
          const lines = t.split("\n");
          if (lines.length <= 15) {
          ...
          // P2: 2000 chars for agent results (research reports etc.)
          out.push({ type: "text", text: `[agent result: ${t.slice(0, 2000)}]` });
        } else {
          // MCP tools and other — keep more context
...
export async function distillSession(inputPath, opts = {}) {
  ...
  // Clear tool ID map for this session
  toolIdMap.clear();
...
if (process.argv[1]?.endsWith("session-distiller-v3.mjs")) {
  const args = process.argv.slice(2);
  const dryRun = args.includes("--dry-run");
  const inputPath = args.find(a => !a.startsWith("--"));

  if (!inputPath) {
    console.error("Usage: node session-distiller-v3.mjs <session.jsonl> [--dry-run]");
    process.exit(1);
  }
