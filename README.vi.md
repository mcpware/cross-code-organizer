...
**Cross-Code Organizer (CCO)** giờ là universal AI coding tool config manager: một dashboard để quản lý config của Claude Code và Codex CLI — MCP server, skills, sessions, profiles, runtime và file project. Sidebar có harness selector để đổi tool.
...
CCO bắt đầu là organizer cho Claude Code. Từ v0.19.0, nó là dashboard cross-harness.

Dùng selector **Harness** trong sidebar để chuyển giữa Claude Code và Codex CLI. Mỗi harness giữ path, category và rule riêng: Claude Code quản lý memories, skills, MCP, commands, agents, hooks; Codex CLI quản lý `~/.codex` config, file AGENTS, skills, MCP servers, profiles, sessions, history, shell snapshots và runtime.

Tiếp theo sẽ thêm Cursor, Windsurf và Aider.
...
2. **Map project scope** — đọc project từ filesystem path rồi map vào model Global/Project của harness đang chọn
...
[ithiria894](https://github.com/ithiria894) — Build tool cho hệ sinh thái AI coding tool.
