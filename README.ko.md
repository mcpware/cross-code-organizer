...
**Cross-Code Organizer (CCO)**는 이제 universal AI coding tool config manager예요. Claude Code와 Codex CLI의 config, MCP 서버, skills, sessions, profiles, runtime, project 파일을 한 대시보드에서 관리하고, sidebar의 harness selector로 tool을 바꿀 수 있어요.
...
CCO는 Claude Code organizer로 시작했어요. v0.19.0부터는 cross-harness dashboard예요.

sidebar의 **Harness** selector로 Claude Code와 Codex CLI를 바꿀 수 있어요. 각 harness는 자기 path, category, rule을 그대로 가져가요. Claude Code는 memories, skills, MCP, commands, agents, hooks를 관리하고, Codex CLI는 `~/.codex` config, AGENTS 파일, skills, MCP servers, profiles, sessions, history, shell snapshots, runtime을 관리해요.

다음으로 Cursor, Windsurf, Aider를 추가할 예정이에요.
...
2. **Project scope 파악** — 파일 시스템 경로에서 project를 찾고 선택한 harness의 Global/Project 모델에 매핑해요
...
[ithiria894](https://github.com/ithiria894) — AI coding tool 생태계 도구 만들고 있어요.
