...
**Cross-Code Organizer (CCO)** は universal AI coding tool config manager。Claude Code と Codex CLI の config、MCP サーバー、skills、sessions、profiles、runtime、project ファイルを1つの dashboard で管理できる。sidebar の harness selector で tool を切り替えられる。
...
CCO は Claude Code organizer として始まった。v0.19.0 からは cross-harness dashboard。

sidebar の **Harness** selector で Claude Code と Codex CLI を切り替えられる。各 harness はそれぞれの path、カテゴリ、ルールを持つ。Claude Code は memories、skills、MCP、commands、agents、hooks を管理し、Codex CLI は `~/.codex` config、AGENTS ファイル、skills、MCP servers、profiles、sessions、history、shell snapshots、runtime を管理する。

次に Cursor、Windsurf、Aider を追加予定。
...
2. **project scope を解決** — filesystem path から project を scan し、選択中 harness の Global/Project model に map
